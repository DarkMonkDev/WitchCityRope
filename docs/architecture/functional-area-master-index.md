...
<!-- Last Updated: 2026-03-18 - Ticket Assignment & Proxy RSVP Added -->
<!-- Version: 3.7 -->
...
| **Events - Ticket Assignment & Proxy RSVP** | `/docs/functional-areas/events/ticket-assignment-proxy-rsvp/` | `/docs/functional-areas/events/ticket-assignment-proxy-rsvp/requirements/` | **NEW FEATURE** - Multi-ticket purchase with assignment to other users, proxy RSVP, "Authorized Contacts" delegate system, pending acceptance with waiver/ToS enforcement. Supports configurable quantity per ticket type (default 3), post-purchase assignment, 4 new email templates (assignment + reminder for both tickets and RSVPs). Research complete, 14 architectural decisions documented. | **Phase 1 - Requirements (In Progress)** | 2026-03-18 |
...
| **Logging & Observability** | `/docs/functional-areas/logging-observability/` | `/docs/functional-areas/logging-observability/` | Site-wide logging infrastructure (Serilog), structured logging, observability, error tracking, and reporting | Not Started | 2026-03-02 |
...
| **Payment Entity Consolidation** | `/docs/functional-areas/payment-entity-consolidation/` | `/docs/functional-areas/payment-entity-consolidation/` | Consolidate dual TicketPurchase/Payment entities, remove dead PaymentFailure code, clean up broken navigation | Not Started | 2026-03-02 |
...
| **Vetting System** | `/docs/functional-areas/vetting-system/` | `/docs/functional-areas/vetting-system/new-work/2026-04-22-bulk-send-reminder/` | **FEATURE SHIPPED TO STAGING** ✅ - Bulk Send Reminder action on the admin vetting list (filters to InterviewApproved, fans out via Promise.allSettled, reuses single-app endpoint + InterviewReminder template) plus a new "Reminders" column showing RemindersSentCount. Side fixes shipped in the same commit: lifted selection state to AdminVettingPage (controlled-component pattern, fixes pre-existing checkbox-not-clearing bug for both Send Reminder and Put On Hold), Rules of Hooks fix in AdminVettingPage, React Query invalidation in OnHoldModal so list auto-refreshes after a hold action. New test infra: `run-test-suite --mode react` for vitest. Commits a5458644 + ed446493. Prior feature in this area: 2025-10-04 conditional "How to Join" menu visibility (production-ready, 46 tests passing). | **FEATURE SHIPPED — STAGING** | 2026-04-22 |
...
- **Objective**: Validate existing vertical slice implementation against latest .NET 10 best practices, identify technical debt, consolidate coding standards
