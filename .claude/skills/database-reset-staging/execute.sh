...
ssh -i $SSH_KEY $USER@$SERVER "cd $DEPLOY_PATH && IMAGE_TAG=latest docker-compose -f docker-compose.staging.yml pull api && IMAGE_TAG=latest docker-compose -f docker-compose.staging.yml up -d --force-recreate api"

if [ $? -ne 0 ]; then
    echo "   ❌ FAIL: Could not pull/recreate API container"
    echo ""
    echo "💡 Manual recovery may be required:"
    echo "   ssh $USER@$SERVER 'cd $DEPLOY_PATH && IMAGE_TAG=latest docker-compose -f docker-compose.staging.yml up -d api'"
    exit 1
fi
