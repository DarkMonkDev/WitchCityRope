...
DB_CONNECTION=$(vault_get_field "secret/projects/witchcityrope/production" "PROD_DB_CONNECTION_STRING")

if [ -z "$DB_CONNECTION" ]; then
    echo "   ❌ FAIL: Could not retrieve database connection string from vault"
    echo ""
    echo "💡 Verify secret exists: vault kv get secret/projects/witchcityrope/production"
    exit 1
fi
