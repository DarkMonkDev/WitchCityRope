...
public class PayPalService : IPayPalService
{
    ...
    public async Task<Result> ProcessWebhookEventAsync(Dictionary<string, object> webhookEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            if (!webhookEvent.TryGetValue("event_type", out var eventTypeObj) || eventTypeObj is not string eventType)
            {
                return Result.Failure("Invalid webhook event: missing event_type");
            }
