        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var notes = auditLogsWithUsers
                // Case-insensitive action matching prevents missing audit log entries from casing differences
                .Where(log => string.Equals(log.Action, "Note Added", StringComparison.OrdinalIgnoreCase) || (log.Action != null && log.Action.Contains("Status Changed", StringComparison.OrdinalIgnoreCase)))
                .Select(log => new ApplicationNoteDto
                {
                    ...
                    Content = IsAutoGeneratedNote(log.Notes)
                        ...
                        : log.Notes ?? "",
                    Type = string.Equals(log.Action, "Note Added", StringComparison.OrdinalIgnoreCase) ? "Note" : "Decision",
                    IsPrivate = true,
