    ...
    private async Task SeedEventsTemplatesAsync(Guid adminUserId, CancellationToken cancellationToken)
    {
        ...
        var templates = new[]
        {
            ...
            new GlobalEmailTemplate
            {
                ...
                Variables = JsonSerializer.Serialize(new[] { "{{attendee_name}}", "{{event_title}}", "{{session_date}}", "{{session_time}}", "{{event_date}}", "{{event_time}}", "{{venue_name}}", "{{venue_address}}", "{{event_details_url}}", "{{event_details_button}}", "{{rsvp_sessions_list}}", "{{rsvp_sessions_list_text}}" }),
                TriggerType = TemplateTriggerType.FixedEvent,
                RecipientGroup = EventRecipientGroup.RSVPTicketHolders,
                IsActive = true,
                Version = 1,
