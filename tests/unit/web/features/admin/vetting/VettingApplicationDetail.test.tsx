...
import { useVettingApplicationDetail } from '@/features/admin/vetting/hooks/useVettingApplicationDetail'
import { useSubmitReviewDecision } from '@/features/admin/vetting/hooks/useSubmitReviewDecision'
import { useApproveApplication } from '@/features/admin/vetting/hooks/useApproveApplication'
import type { ApplicationDetailResponse } from '@/features/admin/vetting/types/vetting.types'

// Mock the hooks
vi.mock('@/features/admin/vetting/hooks/useVettingApplicationDetail')
vi.mock('@/features/admin/vetting/hooks/useSubmitReviewDecision')
vi.mock('@/features/admin/vetting/hooks/useApproveApplication')
...
describe('VettingApplicationDetail', () => {
  ...
  it('renders application details correctly', () => {
    render(<VettingApplicationDetail applicationId="app-1" onBack={mockOnBack} />, {
  ...
  it('shows correct action buttons based on status', () => {
    ...
    // For UnderReview status, should show main action buttons but NOT reminder
    expect(screen.getByText('APPROVE FOR INTERVIEW')).toBeInTheDocument()
    expect(screen.getByText('PUT ON HOLD')).toBeInTheDocument()
    expect(screen.queryByTestId('send-reminder-button')).not.toBeInTheDocument()
  ...
  it('opens send reminder modal when button clicked for InterviewApproved status', async () => {
    // Update mock to use InterviewApproved status
    mockUseVettingApplicationDetail.mockReturnValue({
      data: { ...mockApplication, status: 'InterviewApproved' },
      isLoading: false,
      error: null,
    ...
    const sendReminderButton = screen.getByTestId('send-reminder-button')
  ...
  it('shows error state', () => {
    ...
    expect(
      screen.getByText('Error loading application: Failed to load application')
    ).toBeInTheDocument()
  ...
  describe('Reminder button visibility', () => {
    it('shows reminder button when status is InterviewApproved', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
        data: { ...mockApplication, status: 'InterviewApproved' },
        isLoading: false,
        error: null,
      ...
      const reminderButton = screen.getByTestId('send-reminder-button')
      expect(reminderButton).toBeInTheDocument()
      expect(reminderButton).toBeEnabled()
    })

    it('hides reminder button when status is UnderReview', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
        data: { ...mockApplication, status: 'UnderReview' },
        isLoading: false,
        error: null,
        refetch: mockRefetch,
      })
      ...
      const reminderButton = screen.queryByTestId('send-reminder-button')
      expect(reminderButton).not.toBeInTheDocument()
    })

    it('hides reminder button when status is FinalReview', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
        data: { ...mockApplication, status: 'FinalReview' },
        isLoading: false,
        error: null,
        refetch: mockRefetch,
      })
    ...
    it('hides reminder button when status is Approved', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
        data: { ...mockApplication, status: 'Approved' },
        isLoading: false,
        error: null,
    ...
    it('hides reminder button when status is Denied', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
    ...
    it('hides reminder button when status is OnHold', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
        data: { ...mockApplication, status: 'OnHold' },
        isLoading: false,
        error: null,
        refetch: mockRefetch,
      })
    ...
    it('hides reminder button when status is Withdrawn', () => {
      mockUseVettingApplicationDetail.mockReturnValue({
