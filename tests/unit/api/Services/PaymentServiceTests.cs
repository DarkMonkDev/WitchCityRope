...
public class PaymentServiceTests : IAsyncLifetime
{
    ...
    private PaymentService _sut; // System Under Test
    private ApplicationDbContext _context;
    private Guid _testUserId = Guid.NewGuid();
    private readonly Guid _testRegistrationId = Guid.NewGuid();

    public PaymentServiceTests(DatabaseTestFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task InitializeAsync()
    {
        ...
        // Seed test user with new GUID for each test
    ...
    public async Task ProcessPaymentAsync_WhenPayPalOrderCreationFails_ReturnsFailure()
    {
        ...
        // Verify payment failure was logged to real database
    ...
    {
        // Arrange
        // Create another user for comparison (foreign key requirement)
        var otherUserId = Guid.NewGuid();
        var otherUser = new ApplicationUser
        {
            Id = otherUserId,
            Email = $"other-user-{Guid.NewGuid()}@example.com",
            SceneName = $"OtherUser{Guid.NewGuid().ToString().Substring(0, 8)}",
            EncryptedLegalName = "Encrypted Legal Name",
            DateOfBirth = DateTime.UtcNow.AddYears(-30),
            Role = "Member",
            PronouncedName = "Other User",
            Pronouns = "they/them",
        ...
        {
            EventRegistrationId = Guid.NewGuid(),
            UserId = otherUserId,  // Use created user ID
            AmountValue = 100.00m,
            Currency = "USD",
            Status = PaymentStatus.Completed
        };
