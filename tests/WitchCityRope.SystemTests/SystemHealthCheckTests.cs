...
namespace WitchCityRope.SystemTests;
...
public class SystemHealthCheckTests : IDisposable
{
    ...
    public SystemHealthCheckTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    [Fact]
    [Trait("Priority", "Critical")]
    public async Task VerifyReactAppHasCorrectTitle()
    {
        try
        {
            var response = await _httpClient.GetAsync(_reactUrl);
            ...
            var content = await response.Content.ReadAsStringAsync();
            Assert.Contains("<title>Witch City Rope - Salem's Rope Bondage Community</title>", content);

            _output.WriteLine($"✅ React app has correct title");
        }
        catch (HttpRequestException ex)
        {
            _output.WriteLine($"❌ React app title verification failed");
            _output.WriteLine($"   Error: {ex.Message}");
            throw new InvalidOperationException(
                $"React app title verification failed. Expected: 'Witch City Rope - Salem's Rope Bondage Community'", ex);
        }
    }
    ...
    public async Task RunAllHealthChecks()
    {
        ...
        var healthChecks = new[]
        {
            ...
            ("React App Title", (Func<Task>)VerifyReactAppHasCorrectTitle)
        };
