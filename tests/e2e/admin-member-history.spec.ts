 ...
 *
 * DATABASE DEFENSIVE PATTERN:
 * - Uses unique test user to avoid conflicts with existing data
 * - Cleans up test user after test completes
 */
...
test.describe('Admin Member History Tab', () => {
  test('should display profile changes in History tab after user updates profile', async ({
    ...
    const testUser = await createTestUser({
      email: generateUniqueTestEmail('history-test'),
      password: 'Test123!',
      sceneName: `HistoryTest${Date.now()}`,
    ...
    try {
      ...
      // Get the user ID for later navigation (from testUser object)
      ...
      const timestamp = historyPanel.getByText(/AM|PM/i).first()
  ...
  test('should show empty state when no history exists', async ({ page }) => {
    ...
    // Create a new user with no profile changes
    const testUser = await createTestUser({
      email: generateUniqueTestEmail('history-empty'),
      password: 'Test123!',
      sceneName: `EmptyHistory${Date.now()}`,
  ...
  test('should display multiple profile changes chronologically', async ({
    ...
    // Create test user
    const testUser = await createTestUser({
      email: generateUniqueTestEmail('history-multiple'),
      password: 'Test123!',
      sceneName: `MultiHistory${Date.now()}`,
      membershipLevel: 'Member',
    })
