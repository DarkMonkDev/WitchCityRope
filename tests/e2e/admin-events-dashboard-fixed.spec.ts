...
test.describe('Admin Events Dashboard', () => {
  ...
  test('should filter events by type when unchecking filters', async ({ page }) => {
    ...
    // Get filter chip references - use checkbox input inside chip
    const socialChip = page.locator('input[type="checkbox"][value="Social"]');
    const classChip = page.locator('input[type="checkbox"][value="Class"]');

    // Uncheck Social filter by clicking the label
    const socialLabel = page.locator('label:has-text("Social")').first();
    ...
    const classLabel = page.locator('label:has-text("Class")').first();
