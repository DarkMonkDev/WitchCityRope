...
public abstract class TestBase : TestContext, IDisposable
{
    ...
    protected TestBase()
    {
        ...
        NavigationManager = Services.GetService<FakeNavigationManager>() ?? new FakeNavigationManager();
    ...
    protected virtual void ConfigureTestServices(IServiceCollection services)
    {
        ...
        services.AddScoped<ApiClient>(sp => Mock.Of<ApiClient>());
    }
    ...
    {
        return RenderComponent<TComponent>(builder =>
        {
            foreach (var parameter in parameters)
            {
                builder.Add(parameter.Name, parameter.Value);
            }
        });
    }
